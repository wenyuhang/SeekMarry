...
import static android.renderscript.Allocation.createTyped;
...
public class BlurringView extends ImageView {
    ...
    public BlurringView(Context context, AttributeSet attrs) {
        ...
        final int defaultBlurRadius = res
                .getInteger(R.integer.default_blur_radius);
        final int defaultDownsampleFactor = res
                .getInteger(R.integer.default_downsample_factor);
        final int defaultOverlayColor = res
                .getColor(R.color.default_overlay_color);

        initializeRenderScript(context);

        TypedArray a = context.obtainStyledAttributes(attrs,
                R.styleable.PxBlurringView);
        setBlurRadius(a.getInt(R.styleable.PxBlurringView_blurRadius,
                defaultBlurRadius));
...
//            scaledHeight = scaledHeight - scaledHeight % 4 + 4;
            ...
            mBlurInput = createFromBitmap(mRenderScript,
                    mBitmapToBlur, Allocation.MipmapControl.MIPMAP_NONE,
                    USAGE_SCRIPT);
            mBlurOutput = createTyped(mRenderScript,
                    mBlurInput.getType());
        }
        return true;
    }
